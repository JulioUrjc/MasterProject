...
public:
	Scene9(){
		...
		// { Read Neuron File} Need know layers
		if (neuronFile_ != ""){
			FILE *archivoNeuron;
			...
			weights = init_vector<float>() << 200.0f << 400.0f << 1000.0f << 800.0f << 600.0 << 10.0;
		...
		for (unsigned int numfile = 0; numfile < geomFile_.size(); ++numfile){
			...
			if (geomFile_.at(numfile) == layerBox){
				int offset = 600;
				for (unsigned int layer = 0; layer < layers.size()-1; ++layer){
					loops.push_back(loops.at(loops.size()-1));
					for (unsigned int shape = 0; shape < numGvertex.size(); ++shape){
						geomRead.push_back(vector<b2Vec2>());
						for (unsigned int cont = 0; cont < numGvertex.at(shape); ++cont)
						{
							x = geomRead.at(geomRead.size() - 2).at(cont).x;
							y = geomRead.at(geomRead.size() - 2).at(cont).y;
							x += offset;
							maxX = max(x, maxX);
							minX = min(x, minX);
							maxY = max(y, maxY);
			...
			if (emiters > 0){
				...
				for (unsigned int cont = 0; cont < emiters; ++cont){
					...
					fscanf(archivo, "%f", &x);
					rEmit.SetEmitRate(weights.at(0));
				...
				if (geomFile_.at(numfile) == layerBox){
					...
					for (unsigned int layer = 0; layer < layers.size() - 1; ++layer){
						...
						rEmit.SetParticleSystem(m_particleSystem);
						b2Vec2 pos = m_emitters.at(m_emitters.size() - 1).GetPosition();
						rEmit.SetPosition(pos+b2Vec2(offset, 0.0f));
						b2Vec2 vel = m_emitters.at(m_emitters.size() - 1).GetVelocity();
						rEmit.SetVelocity(vel);
						rEmit.SetSize(b2Vec2(0.0f, faucetLength));
						b2ParticleColor col = m_emitters.at(m_emitters.size() - 1).GetColor();
						rEmit.SetColor(col + b2ParticleColor(col.r+30, col.g+50, col.b+60, 255));
						rEmit.SetEmitRate(weights.at(layer + 1));
						rEmit.SetParticleFlags(TestMain::GetParticleParameterValue());
	...
	virtual void Step(Settings* settings){
		...
		int offset = 10;
		for (unsigned i = 0; i < layers.size(); ++i){
			m_debugDraw.DrawString(offset, 125, layers.at(i).substr(0, layers.at(i).size() - 1).c_str());
			offset += 160;
		}
	}
	...
	static Test* Create(){ return new Scene9; }
...
const float32 Scene9::k_faucetLength = 2.0f;
const float32 Scene9::k_faucetWidth = 0.1f;
const float32 Scene9::k_faucetHeight = 15.0f;

const float32 Scene9::k_containerWidth = 2.0f;
const float32 Scene9::k_containerHeight = 5.0f;

const float32 Scene9::k_containerHalfWidth = Scene9::k_containerWidth / 2.0f;
const float32 Scene9::k_containerHalfHeight = Scene9::k_containerHeight / 2.0f;
const float32 Scene9::k_barrierHeight = Scene9::k_containerHalfHeight / 100.0f;
const float32 Scene9::k_barrierMovementIncrement = Scene9::k_containerHalfHeight * 0.1f;


const int32 Scene9::k_maxParticleCount = 10000;
const ParticleParameter::Value Scene9::k_paramValues[] = {
...
const ParticleParameter::Definition Scene9::k_paramDef[] = { { Scene9::k_paramValues, B2_ARRAY_SIZE(Scene9::k_paramValues) }, };
const uint32 Scene9::k_paramDefCount = B2_ARRAY_SIZE(Scene9::k_paramDef);
