...
class Scene2 : public Test{
	public:
		Scene2(){
			...
			// Initialize the particle emitter1.
			{
				...
				m_emitter.SetPosition(b2Vec2(0.0f,4.85f));
				m_emitter.SetVelocity(b2Vec2(0.1f, 0.0f));
				m_emitter.SetSize(b2Vec2(0.0f, faucetLength));
				m_emitter.SetColor(b2ParticleColor(0, 0, 0, 255));
				m_emitter.SetEmitRate(180.0f);
		...
		b2ParticleColor colorPorcentaje(int p){
			int intervalo = (p>-1) + (p>20) + (p>40) + (p>60) + (p>80);
			switch (intervalo){
	...
	static Test* Create(){ return new Scene2; }
...
const float32 Scene2::k_faucetLength = 2.0f;
const float32 Scene2::k_faucetWidth = 0.1f;
const float32 Scene2::k_faucetHeight = 15.0f;

const float32 Scene2::k_containerWidth = 2.0f;
const float32 Scene2::k_containerHeight = 5.0f;

const float32 Scene2::k_containerHalfWidth = Scene2::k_containerWidth / 2.0f;
const float32 Scene2::k_containerHalfHeight = Scene2::k_containerHeight / 2.0f;
const float32 Scene2::k_barrierHeight = Scene2::k_containerHalfHeight / 100.0f;
const float32 Scene2::k_barrierMovementIncrement = Scene2::k_containerHalfHeight * 0.1f;

const int32 Scene2::k_maxParticleCount = 10000;
const ParticleParameter::Value Scene2::k_paramValues[] = {
...
const ParticleParameter::Definition Scene2::k_paramDef[] = { { Scene2::k_paramValues, B2_ARRAY_SIZE(Scene2::k_paramValues) }, };
const uint32 Scene2::k_paramDefCount = B2_ARRAY_SIZE(Scene2::k_paramDef);
