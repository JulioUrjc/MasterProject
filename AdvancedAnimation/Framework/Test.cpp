...
class QueryCallback : public b2QueryCallback{
	public:
		QueryCallback(const b2Vec2& point){ m_point = point; m_fixture = NULL; }
...
public:
	QueryCallback2(b2ParticleSystem* particleSystem, const b2Shape* shape, const b2Vec2& velocity){
	...
	bool ReportParticle(const b2ParticleSystem* particleSystem, int32 index){
		if (particleSystem != m_particleSystem)	return false;
...
void Test::MouseUp(const b2Vec2& p){
	...
	if (m_bombSpawning){ CompleteBombSpawn(p); }
}
...
void Test::Step(Settings* settings){
	...
	if (settings->pause){
		if (settings->singleStep){	settings->singleStep = 0; }else{ timeStep = 0.0f; }
...
void Test::ColorParticleGroup(b2ParticleGroup * const group, uint32 particlesPerColor){
	...
	if (!particlesPerColor){
		...
		if (!particlesPerColor){ particlesPerColor = 1; }
	}
...
void Test::InitializeParticleParameters(const uint32 filterMask){
	const uint32 defaultNumValues =	ParticleParameter::k_defaultDefinition[0].numValues;
	const ParticleParameter::Value * const defaultValues = ParticleParameter::k_defaultDefinition[0].values;
	...
	for (uint32 i = 0; i < defaultNumValues; i++){
		if (defaultValues[i].value & filterMask){ continue; }
