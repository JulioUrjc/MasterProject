...
const uint32 ParticleParameter::k_DefaultOptions = OptionDrawShapes | OptionDrawParticles;
...
bool ParticleParameter::Changed(bool * const restart){
	...
	m_changed = false;
	if (restart){ *restart = changed && GetRestartOnChange(); }
	return changed;
}
...
const ParticleParameter::Value*	ParticleParameter::FindParticleParameterValue() const{
	uint32 start = 0;
